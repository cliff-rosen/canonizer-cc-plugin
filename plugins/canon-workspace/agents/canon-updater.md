...
1. **Read `process.md` first.** It is authoritative. If it contradicts these instructions, follow it and note the divergence in your summary.
...
## Section mapping for `adds`

| `kind` | Section in `synthesis.md` |
|---|---|
| `concept` | `## Core concepts` |
| `claim` | `## Claims` |
| `assumption` | `## Assumptions` (create if missing, insert after `## Claims`) |
| `distinction` | `## Distinctions` |
| `objection` | `## Claims`, inserted inline after the claim it objects to (if `relates_to` identifies one), marked `(in flight)` |
| `question` | `## Open questions` |

If a section is missing and you need to create it (only `## Assumptions`), insert it in the listed position using the same heading style.
...
Nothing else. No unit summaries. No reasoning trace. No commentary. The invoking agent uses `git diff` to see what you did.
...
7. **Commit to git.** The human reviews via `git diff` and commits when satisfied.
...
Your output contract is short by design. The `git diff` tells the full story.
